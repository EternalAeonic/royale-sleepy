    ...
    task.on(
      ...
      (err) => {
        ...
        if (err.code === 'storage/unauthorized') {
          setError('PERMISSION DENIED: You need to update Firebase rules. Go to console.firebase.google.com -> Storage -> Rules, and set "allow read, write: if true;"');
        } else {
        ...
        {uploading ? (
          ...
          <div className="space-y-3">
            ...
            <p className="font-body text-sm font-semibold text-bark">
              {dragging ? 'Drop your video here!' : 'Click or Drag & Drop your video'}
            </p>
            <p className="font-body text-[10px] text-stone/30 mt-2">MP4, WebM, OGG supported (Max 100MB)</p>
          </div>
        )}
      </div>
